...
import { extractAccession, extractCellText, stripWordXml } from "@/lib/utils/sanitize-text";
...
/** Extract table rows from docx XML — one string per table cell (not per text run). */
...
export async function ingestSperMD(): Promise<IngestRecord[]> {
  ...
  const catalog = generateSperMDCatalog(now);
  const byAccession = new Map(catalog.map((r) => [r.accession, r]));
  ...
  try {
    ...
    if (res.ok) {
      ...
      console.log(`  SperMD docx: merged ${parsed.length} parsed rows into ${byAccession.size} entries`);
    }
  } catch (err) {
    console.warn("  SperMD docx parse skipped:", err);
  }
